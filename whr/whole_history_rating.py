...
class Base:
    ...
    def _setup_game(
        ...
        if extras is None:
            extras = {}
        side_a_list = [side_a] if isinstance(side_a, str) else side_a
        side_b_list = [side_b] if isinstance(side_b, str) else side_b
        
        if set(side_a_list) == set(side_b_list):
            raise AttributeError("Invalid game (side a players == side b players)")
            
        side_a_players = [self.player_by_name(p) for p in side_a_list]
        side_b_players = [self.player_by_name(p) for p in side_b_list]
        game = Game(side_a_players, side_b_players, winner, time_step, handicap, weight, extras)
        return game
    ...
    def create_game(
        ...
        Args:
            side_a (str | list[str]): The name(s) of the side A player(s).
            side_b (str | list[str]): The name(s) of the side B player(s).
            winner (str): "A" if side A won, "B" if side B won.
        ...
        if extras is None:
            extras = {}
        if self.config["uncased"]:
            side_a = [p.lower() for p in side_a] if isinstance(side_a, list) else side_a.lower()
            side_b = [p.lower() for p in side_b] if isinstance(side_b, list) else side_b.lower()
        game = self._setup_game(side_a, side_b, winner, time_step, handicap, weight, extras)
        return self._add_game(game)

    def _add_game(self, game: Game) -> Game:
        for p in game.side_a_players:
            p.add_game(game)
        for p in game.side_b_players:
            p.add_game(game)

...
def main():
    ...
    # Filter for files containing '2023-'
    files = [f for f in files if "2023-" in f]
    print(f"Found {len(files)} files from 2023 to parse in {INPUT_DIR}.")
    
    parsed_count = 0
    # Sort in reverse order so the newest (September 2023) is parsed first!
    for filename in sorted(files, reverse=True):
        ...
        if rank_date in parsed_dates:
            print(f"Date {rank_date} already parsed, skipping.")
            continue
